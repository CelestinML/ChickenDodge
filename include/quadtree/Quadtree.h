...
// cf : https://github.com/pvigier/Quadtree
...
template<typename T, typename GetBox, typename Collides, std::size_t Threshold = 16, std::size_t MaxDepth = 8, typename Equal = std::equal_to<T>, typename Float = float>
class Quadtree
{
    /*
    ...
    static_assert(std::is_convertible_v<std::invoke_result_t<Collides, const T&, const T&>, bool>,
        "Collides must be a callable of signature bool(const T&, const T&)");
...
public:
    Quadtree(const Box<Float>& box, 
        const GetBox& getBox = GetBox(),
        const Collides& collides = Collides(),
        const Equal& equal = Equal()) :
        mBox(box), mRoot(std::make_unique<Node>()), mGetBox(getBox), mCollides(collides), mEqual(equal)
    {
...
private:
    ...
    void findAllIntersections(Node* node, std::vector<std::pair<T, T>>& intersections) const
    {
        ...
        for (auto i = std::size_t(0); i < node->values.size(); ++i)
        {
            ...
            {
                if (mCollides(node->values[i], node->values[j]))
