...
namespace SimpleGE::Network
{
  ...
  {
    int one = 1;
    int result = ::setsockopt(clientSocket, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<const char*>(&one), sizeof(one));
  ...
  void Connection::Service()
  {
    ...
    std::size_t length{};
    RecvRaw(gsl::span<std::byte>(reinterpret_cast<std::byte*>(&length), sizeof(length)));
  ...
  {
    auto bytesReceived = ::recv(clientSocket, reinterpret_cast<char*>(buffer.data()), buffer.size(), 0);
  ...
  {
    auto bytesSent = ::send(clientSocket, reinterpret_cast<char*>(data.data()), data.size_bytes(), 0);
  ...
  std::shared_ptr<Server> Subsystem::Listen(std::uint16_t port, const Server::OnConnection& onConnection,
                                            ...
                                            const Connection::OnData& onData)
  {
    ...
    int option = 1;
    result = ::setsockopt(listening, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<char*>(&option), sizeof(option));
    if (result == SOCKET_ERROR)
    {
      int err = GetSockError();
      fmt::print(stderr, "{}\n", GetErrorStr(err));
  ...
  std::shared_ptr<Connection> Subsystem::Connect(std::string_view host, std::uint16_t port,
                                                 const Connection::OnDisconnect& onDisconnect,
                                                 const Connection::OnData& onData)
  {
