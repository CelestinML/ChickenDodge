...
namespace ChickenDodge
{
  ...
  {
    // impl->score->RegisterScoreChangedEvent([this](int score){
    //   NetworkScore msg = NetworkScore(score);
  ...
  void PlayerComponent::OnCollision(const SimpleGE::ColliderComponent& other)
  {
    ...
    if (rupee != nullptr)
    {
      ...
      NetworkScore msg = NetworkScore(impl->score->GetValue());
      NetworkSystem::Send(msg);
  ...
  void PlayerComponent::SetLocal(bool isLocal) { impl->isLocal = isLocal; }

  void PlayerComponent::SetName(std::string_view name) { impl->name = name; }
